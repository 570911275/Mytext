...
{
    $user_in = "insert into user_info(username,password,email,regdate,power)values('$username','$password','$email','$regdate','$power')";
    $result=mysqli_query($conn,$user_in) or die(mysqli_error($conn));
    if($result)
    {
    echo "<script>alert('前往验证');location.href='codePicture.html';</script>";
    }
    else die('插入数据库失败');
}
