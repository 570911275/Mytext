...
$id = $_SESSION['userid'];
$status = 2;
$label = Order::findAll(['status'=>$status,'storeid'=>$id]);
$label_length = count($label);
if($label_length==0)
{
  echo "<script>alert('目前，尚无未处理订单');history.go(-1);</script>";
}
?>
...
<?php for($i=0;$i<$label_length;$i++){?>
            ...
            <button type="button"><a href="storeOrderUndoControl.php?id=<?php echo $label[$i]->id?>">确认发货</a></button>
            <hr/>
            <hr/>
<?php }?>
</p>
