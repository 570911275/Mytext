...
$label  = User::findAll(['power'=>3]);
$label_length = count($label);

if($label_length==0)
{
    echo "<script>alert('您好，目前尚无商家申请商铺资格！');history.go(-1);</script>";
}
?>
...
<?php for($i=0;$i<$label_length;$i++)  {?>
    ...
    <fieldset>
            ...
            <h3>商铺名称：<?php echo $label[$i]->store_name ?> </h3>
            <button type="button"><a href="storeAuditControl.php?id=<?php echo $label[$i]->id?>">审核通过</a></button>
            <button type="button"><a href="storeAuditUnControl.php?id=<?php echo $label[$i]->id?>">审核失败</a></button>
            </div>
    </fieldset>
    </div>
<?php } ?>
