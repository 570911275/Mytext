...
//获取id
$storeid = $_SESSION['chat_storeid'];
$bookid = $_SESSION['chat_bookid'];
$userid = $_SESSION['userid'];
if($storeid==$userid)
{
    echo "<script>alert('您好，您为该店店主，无需询问自己！');history.go(-1);</script>";
    exit;
}
//获取数据
...
<legend>YY书城——联系商家</legend>
    ...
    <div class="talk_con">
        <div class="talk_show">
            <div class="atalk"><span id="asay">您好丫，欢迎光临,很高兴为您服务</div>
            <?php for($i=0;$i<$label_length;$i++){?>
            <div class="btalk"><span id="bsay"><?php echo $label[$i]->comment?></div>
            <?php 
            $id = $label[$i]->id;
            $label_reply = Chat::findOne(['reply_id'=>$id]);
            ?>
            <div class="atalk"><span id="asay"><?php 
