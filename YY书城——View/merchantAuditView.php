...
$label  = User::findAll(['power'=>2]);
$label_length = count($label);

if($label_length==0)
{
    echo "<script>alert('您好，目前尚无用户申请商家资格！');history.go(-1);</script>";
}
...
<?php for($i=0;$i<$label_length;$i++)  {?>
    ...
    <fieldset>
            ...
            <button type="button"><a href="merchantAuditControl.php?id=<?php echo $label[$i]->id?>">审核通过</a></button>
            <button type="button"><a href="merchantAuditUnControl.php?id=<?php echo $label[$i]->id?>">审核失败</a></button>
            </div>
    </fieldset>
    </div>
<?php } ?>
